...
function formatWithHeaderFooter(message) {
    try {
        ...
        return `🏢 *MJNet BOT MANAGEMENT ISP*\n\n${message}\n\nInternet Tanpa Batas`;
    }
}
...
async function connectToWhatsApp() {
    try {
        ...
        sock = makeWASocket({
            auth: state,
            logger,
            browser: ['MJNet DIGITAL NETWORK', 'Chrome', '1.0.0'],
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'open') {
                ...
                try {
                    // Pesan notifikasi
                    const notificationMessage = `📱 *BOT WHATSAPP MJNet*\n\n` +
...
async function sendWelcomeMessage(remoteJid, isAdmin = false) {
    try {
        ...
        let welcomeMessage = `👋 *Selamat Datang di Bot WhatsApp ${getSetting('company_header', 'MJNet BOT MANAGEMENT ISP') || 'ISP Monitor'}*\n\n`;
        ...
        // Tambahkan footer
        welcomeMessage += `🏢 *${getSetting('company_header', 'MJNet BOT MANAGEMENT ISP') || 'ISP Monitor'}*\n`;
...
async function handleAdminMenu(remoteJid) {
    try {
        ...
        // Tambahkan footer
        adminMessage += `🏢 *${getSetting('company_header', 'MJNet BOT MANAGEMENT ISP') || 'ISP Monitor'}*\n`;
